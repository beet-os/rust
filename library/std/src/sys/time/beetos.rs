use crate::os::beetos::ffi::blocking_scalar;
use crate::os::beetos::services::TicktimerScalar::ElapsedMs;
use crate::os::beetos::services::ticktimer_server;
...
impl SystemTime {
    ...
    pub fn now() -> SystemTime {
        // Use ticktimer elapsed as a monotonic clock for now.
        // A proper RTC/systime service can be added later.
        let result = blocking_scalar(ticktimer_server(), ElapsedMs.into())
            .expect("failed to request elapsed_ms");
        let lower = result[0];
        let upper = result[1];
